<template>
  <div class="servicios-container">
    ...
    <div class="header">
      ...
      <BaseButton @click="openModal()" variant="primary" size="md" class="btn-add-header">
    ...
    <div v-else-if="services.length === 0" class="empty-state">
      ...
      <BaseButton @click="openModal()" variant="primary" size="md" class="btn-empty">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
          <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
        </svg>
    ...
    <div v-else class="services-grid">
      <BaseCard v-for="service in services" :key="service.id" class="service-card" :padded="false">
        ...
        <div class="service-actions">
          <BaseButton @click="editService(service)" variant="secondary" size="sm" class="btn-edit">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
              <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
            </svg>
            Editar
          </BaseButton>
          <BaseButton @click="deleteService(service.id)" variant="danger" size="sm" class="btn-delete">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-4 h-4">
    ...
    <div v-if="showModal" class="modal-overlay" @click="closeModal()">
      <div class="modal-content" @click.stop>
        ...
        <form @submit.prevent="saveService" class="modal-body">
          ...
          <BaseButton type="submit" variant="primary" size="lg" full-width class="btn-submit">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
...
} from '@/domains/services/api/servicesApi';
...
const saveService = async () => {
  try {
    ...
    if (editingService.value) {
      ...
      res = await updateServiceRequest(editingService.value.id, newService.value);
      ...
      res = await createServiceRequest({ ...newService.value });
...
const deleteService = async (id) => {
  const confirmed = await confirmDialog('¿Estás seguro de eliminar este servicio?', 'Eliminar servicio');
  if (!confirmed) return;
  try {
